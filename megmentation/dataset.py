...
class PolygonSegmentationDataset(Dataset):
    ...
    def _load_image_and_mask(self, image_path, annotation):
        ...
        mask = cv2.resize(mask, self.target_size, interpolation=cv2.INTER_NEAREST)  # Use NEAREST to preserve label values
    ...
    def __getitem__(self, idx):
        ...
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        image = cv2.resize(image, self.target_size)
        image = torch.from_numpy(image).permute(2, 0, 1).float() / 255.0  # Scale to [0, 1]
        
        # Ensure the mask is a NumPy array
        if not isinstance(mask, np.ndarray):
            raise ValueError(f"Expected mask to be a NumPy array, but got {type(mask)}")

        # Convert the mask to a tensor and ensure it's of the correct type
